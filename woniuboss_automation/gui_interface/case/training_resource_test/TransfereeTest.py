...
from gui_interface.common.training_resource.Transferee import Transferee
from gui_interface.util.Service import Service
from gui_interface.util.Utility import Utility
import re


# 数据
transferee_data = Utility.read_json("../../test_data/training_resource/transferee_data")
        ...
        if query_result != "无符合条件的记录":
            ...
            # 默认选中第一条，还有一页多少条数据显示，还有翻页功能
                ...
                # 不分页
                    ...
                    # 选中第一个
                    Service.get_tier_ele(query_result_count[0], By.XPATH,
                                         "//table[@id='transmit-table']/tbody/tr[1]/input").click()
                    # 本页最后一个
                    # 本页最后一个
