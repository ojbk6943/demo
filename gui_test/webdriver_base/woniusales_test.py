...
class WoniuSalesTest:
    ...
    def __init__(self,browser,url):
        ...
        if browser == 'Chrome':
            ...
            self.driver = webdriver.Firefox()
        self.db_info = ('192.168.2.17', 'root', '123456', 'woniusales')
    ...
    def add_woniusales_cookie(self):
        ...
        self.driver.add_cookie({'name': 'password', 'value': 'admin'})
        self.driver.get('http://192.168.2.17:8080/WoniuSales/')
    ...
    def test_login(self):
        login_data = [('admin','admin','login_manager-pass'),('admin','admin1','login_manager-fail'),
                      ('admin1','admin','login_manager-fail'),('','admin','login_manager-fail'),('admin','','login_manager-fail')]

        print('登录测试结果：')
        for login_info in login_data:
            self.login(login_info)
            if self.is_element_present(By.LINK_TEXT,'注销'):
                actual = 'login_manager-pass'
                self.driver.find_element_by_partial_link_text('注销').click()
            else:
                actual = 'login_manager-fail'
                time.sleep(2)
                self.driver.refresh()
    ...
    def test_add_customer(self):
        # customer_data = [('18000002','小明','2005-03-01','100','200','add-customer-success'),
        #                  ('','','2001-01-01','','','add-customer-fail'),('18000002','小明','2005-03-01','100','200','add-customer-fail')]
    ...
    def edit_customer(self,edit_info):
        ...
        # 修改客户信息
        time.sleep(2)
        driver.find_element_by_xpath('//tbody[@id="customerlist"]/tr[1]/td[11]/a').click()
        customer_phone = driver.find_element_by_id('customerphone')
        customer_phone.click()
        customer_phone.clear()
