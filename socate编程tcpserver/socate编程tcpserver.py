...
HOST = '0.0.0.0'     #0.0.0.0指绑定主机上所有可用地址
PORT = 28567           #65535
BUFSIZ = 1024   #定义一次从socket缓冲区最多读入1024个字节数据
ADDR = (HOST, PORT)

#创建socket，指明网络层协议,AF_INET表示是IPv4协议,  SOCK_STREAM表示传输层是tcp协议
tcpSerSock = socket(AF_INET, SOCK_STREAM)         #创建socket类的实例对象

#绑定地址和端口，表示使用这个地址
# HOST为空字符串表示 本机所有ip， port为28567
tcpSerSock.bind(ADDR)

#使socket处于监听状态，参数大意是指，允许等待连接的客户端的最大数量
# 这个TCP 服务进程监听在 本机所有ip， port为28567，等待客户端的连接。
tcpSerSock.listen(6)
